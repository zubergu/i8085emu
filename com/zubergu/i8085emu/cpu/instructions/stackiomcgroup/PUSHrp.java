...
public class PUSHrp implements Instruction {
	private Cpu cpu;
	private RegisterPairName rpName;
	
	public PUSHrp(Cpu cpu, RegisterPairName rpName) {
		this.cpu = cpu;
		this.rpName = rpName;
	}
	...
	public int execute() {
		...
		sp.decrement();
		memory.writeByte(sp.getValue(), rpH.getValue());
		sp.decrement();
		memory.writeByte(sp.getValue(), rpL.getValue());
