...
public class RconditionTest {
	
	@Test
	public void correctInstructionExecutionCarryNotSet() {
		...
		Rcondition instruction = new Rcondition(cpu, JumpCondition.C);
		
		Assert.assertEquals(instruction.execute(), 6);
		
		verify(sp, never()).increment();
	...
	@Test
	public void correctInstructionExecutionCarrySet() {
		Cpu cpu = Mockito.mock(Cpu.class);
		Memory m = Mockito.mock(Memory.class);
		Register16Bit pc = Mockito.mock(Register16Bit.class);
		Register16Bit sp = Mockito.mock(Register16Bit.class);
