...
public class SBBMTest {
	...
	public void correctInstructionCarrySet() {
		...
		when(hl.getValue()).thenReturn(0x000A);
		when(m.readByte(0x000A)).thenReturn(0x03);
		
		SBBM instruction = new SBBM(cpu);
		
		Assert.assertEquals(instruction.execute(), 7);
		verify(a).setValue(0x03);
		verify(cpu).setCarryFlag(0x07, 0x04, true);
		verify(cpu).setAuxiliaryCarryFlag(0x07, 0x04, true);
		verify(cpu).setZeroFlag(0x03);
		verify(cpu).setSignFlag(0x03);
		verify(cpu).setParityFlag(0x03);
	}
